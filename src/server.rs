...
use std::{collections::{HashMap, HashSet}, io, sync::{atomic::{AtomicUsize, Ordering}, Arc}};
...
pub struct ChatServer {
    /// Map of connection IDs to their message receives
    sessions: HashMap<ConnId, mpsc::UnboundedSender<Msg>>,

    /// Map of room name to participant connection IDs in that room
...
impl ChatServer {
    ...
    async fn send_message(&self, conn: ConnId, msg: impl Into<String>) {
        if let Some(room) = self.rooms.iter().find_map(|(room, participants)| participants.contains(&conn).then_some(room)) {
    ...
    /// Unregister connection from room map and braodcast disconnection message
...
impl ChatServerHandle {
    ...
    pub async fn join_room(&self, conn: ConnId, room: impl Into<String>) {
        ...
        // unwrap: chat server should not have been dropped
        self.cmd_tx.send(Command::Join { conn, room: room.into(), res_tx, }).unwrap();
    ...
    pub async fn send_message(&self, conn: ConnId, msg: impl Into<String>) {
        ...
        // unwrap: chat server should not have been dropped
        self.cmd_tx.send(Command::Message { msg: msg.into(), conn, res_tx, }).unwrap();
